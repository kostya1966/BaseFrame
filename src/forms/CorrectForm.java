...
public class CorrectForm extends Formr {
...
//        }
    ...
    public void LOAD_OBJ() {
        ...
        for (int i = 0; i < V.PARAMTO.length / 4; i++) {
            ...
            F[i] = P.addobjF(this, "F" + i, V.PARAMTO[k + 3], 250, 30);
            if (!V.PARAMTO[k + 1].equals("")) {
                F[i].SETTYPE(V.PARAMTO[k + 1]);
            }
            if (!V.PARAMTO[k + 2].equals("")) {
                F[i].SETINPUTMASK(V.PARAMTO[k + 2]);
            }
            F[i].setText(V.PARAMTO[k + 3]);
            if (V.PARAMTO[k + 2].contains(".") && !V.PARAMTO[k + 3].contains(".") && V.PARAMTO[k + 1].equals(V.TYPE_NUMERIC)) {		
                F[i].setText(V.PARAMTO[k + 3] + ".0");		
            }
            k += 4;
        }
    ...
    public void LOC_ABOUT() {
        ...
        for (int i = 0; i < arr.size() / 4; i++) {
            if (i == 0) {
                ...
                locate(F[i], L[num], V.LOC_RIGHT, 5, null, V.LOC_UP, 10);
            } else {
                locate(L[i], null, V.LOC_LEFT, 5, L[i - 1], V.LOC_DOWN, 15);
                locate(F[i], L[num], V.LOC_RIGHT, 5, L[i - 1], V.LOC_DOWN, 10);
            }
        }
        locate(B[0], null, V.LOC_LEFT, 5, null, V.LOC_DOWN, 10);
        locate(B[1], null, V.LOC_RIGHT, 5, null, V.LOC_DOWN, 10);
        setBounds(getX(), getY(), L[num].getWidth() + F[numF].getWidth() + 30, arr.size() / 4 * 40 + 80);
    }

...
public class DigitsToTextConverter {
    ...
    public static String digits2text(Double d) {
        ...
        String s = (new DecimalFormat("#.00", decimalFormatSymbols)).format(d);
        int n = s.length() - s.lastIndexOf('.');
        if (d == null || d < 0.0 || d > 1000000000.0 || n > 3) {
...
//            threeChars.push(new ThreeChar());
//            threeChars.peek().range = Ranges.UNITS;
//            sb = new StringBuilder(sa[1]).reverse();
//            for (int i = 0; i < sb.length(); i++) {
//                if (i > 0 && i % 3 == 0) {
//                    threeChars.push(new ThreeChar());
//                }
...
//            }
        ...
        switch (sa[1].charAt(1)) {
            case '1':
                result.append(sa[1].charAt(0) != '1' ? "коп." : "коп.");
                break;
            case '2':
            case '3':
            case '4':
